...
public class RodTrailsMenu extends Menu {

    public RodTrailsMenu() {
        super("&6&nRod Trails Menu", 2);
    }
    ...
    public void buildButtons(Player player) {
        ...
        for(RodTrail rodTrail : trailsManager.getRodTrails()) {
            inventory.setItem(rodTrail.getSlot(), rodTrail.getMenuItem());
        }
        ...
        inventory.setItem(13, ItemBuilder.construct().create(Material.FEATHER, 1)
                .setDisplay("&b&lClick to unequip your rod trail").build());
    }
    ...
    public void handleOnClick(InventoryClickEvent e) {
        ...
        RodTrail t = null;
        for(RodTrail trail : manager.getRodTrails()) {
        ...
        if(t != null) {
            ...
            if(item.getType() == Material.FEATHER) {
                ...
                if(data.getRodTrail() != null) {
                    data.setRodTrail(null);
                    userManager.updateCached( data);
                    Formatter.sendMsg(player, "&eYou have unequipped your rod trail");
                }else {
                    Formatter.sendMsg(player, "&cYou don't have a rod trail !");
                }
            }
        }
        e.setCancelled(true);
